...
import { Plus, Target, Coins, TrendingUp, Users } from 'lucide-react';
import { format } from 'date-fns';

export function Dashboard({ onSelect }: { onSelect: (id: string) => void }) {
  const { state, addBeneficiary } = useStore();
        ...
        <div>
          ...
          {state.currentGoldPricePerUnit ? (
            <p className="text-sm font-medium text-amber-600 mt-1 flex items-center gap-1.5">
              <Coins className="w-4 h-4" /> Live 21K English Coin (8g): {formatCurrency(state.currentGoldPricePerUnit, state.currency)}
            </p>
          ) : (
            <p className="text-sm text-stone-500 mt-1">Overview of your family's savings.</p>
          )}
        </div>
        <button 
          onClick={() => setShowAdd(true)}
          className="flex items-center gap-2 bg-stone-900 hover:bg-stone-800 text-white px-5 py-2.5 rounded-full font-medium transition-all text-sm shadow-sm hover:shadow active:scale-95"
        >
